...
async function mergeSort(head: ListNode | null): Promise<ListNode | null> {
  ...
  if (head.next.next == null)
    middle = head; // Special case when the list has only two elements.
...
function arrayToList(arr: number[]): ListNode | null {
  ...
  for (let val of arr) {
    if (listLength == 5e4)
      throw new Error("Number of values should be less than 15e4");

    if (val < -1e5 || val > 1e5)
      throw new Error(
        `Invalid node value ${val}. Node values should be between -10^5 and 10^5.`
      );
  ...
  if (listLength == 0) throw new Error("Should be minimum 1 value");
